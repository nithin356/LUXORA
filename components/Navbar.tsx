...
const Navbar: React.FC<NavbarProps> = ({ activePage, setPage }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled || isMenuOpen ? 'bg-luxora-dark/95 backdrop-blur-md py-2 sm:py-3 shadow-2xl' : 'bg-transparent py-3 sm:py-5'}`}>
      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 flex justify-between items-center">
        ...
        >
          <img src="/logo_Lg.png" alt="Luxora Premier Ecosystem" className="h-8 sm:h-10 md:h-14 lg:h-16 w-auto object-contain flex-shrink-0 transform group-hover:scale-105 transition-transform" />
          <span className="text-sm sm:text-lg md:text-2xl lg:text-3xl font-futuristic gold-text tracking-[0.1em] sm:tracking-[0.2em] font-bold group-hover:drop-shadow-[0_0_15px_rgba(212,175,55,0.5)] transition-all truncate">LUXORA</span>
        </div>
        ...
        <div className="md:hidden z-50">
            ...
            >
                {isMenuOpen ? (
                  ...
                  <svg className="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                )}
            </button>
        </div>
      </div>
      ...
      >
          <div className="flex justify-between items-center p-4 sm:p-6 border-b border-white/5 bg-transparent">
              <div className="flex items-center gap-2 min-w-0">
                <img src="/logo_Lg.png" alt="Luxora Logo" className="h-8 sm:h-10 w-auto object-contain flex-shrink-0" />
                <span className="text-sm sm:text-lg md:text-xl font-futuristic gold-text tracking-[0.15em] sm:tracking-[0.2em] font-bold truncate">LUXORA</span>
              </div>
              <button 
                ...
                {navItems.map((item, index) => (
                  <button
                    ...
                    className={`text-base sm:text-lg md:text-xl uppercase tracking-[0.3em] sm:tracking-[0.4em] font-serif transition-all duration-700 text-left relative group ${
                      activePage === item.value ? 'text-luxora-gold pl-4 sm:pl-6' : 'text-white/60 hover:text-white pl-0'
                    }`}
                    style={{ 
            ...
            <div className="flex flex-col items-center space-y-4 sm:space-y-6">
              <div className="flex space-x-6 sm:space-x-8">
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-luxora-gold transition-colors">
                  <span className="text-[9px] sm:text-[10px] uppercase tracking-[0.3em] font-bold">Instagram</span>
                </a>
                <a href="#" className="text-white/40 hover:text-luxora-gold transition-colors">
                  <span className="text-[9px] sm:text-[10px] uppercase tracking-[0.3em] font-bold">WhatsApp</span>
                </a>
              </div>
              <div className="text-center">
                <p className="text-[8px] sm:text-[9px] uppercase tracking-[0.6em] text-luxora-gold font-bold mb-1">Bengaluru, India</p>
                <p className="text-[7px] sm:text-[8px] uppercase tracking-[0.4em] text-white/20">The Premier Ecosystem</p>
              </div>
            </div>
          </div>
      </div>
